...
#Variables to automate netmaker installation
GET_EMAIL=netmakerdomain@test.com
...
set_install_vars() {
	...
	echo "----------------------------------------------------"
	echo "Showing MASTER_KEY to curl HTTP request"
	echo $MASTER_KEY

	DOMAIN_TYPE="custom"

	echo "-----------------------------------------------------"
	echo "Using custom domain: $NETMAKER_BASE_DOMAIN"
	echo "Also the following subdomains will be used:"
	echo "          dashboard.$NETMAKER_BASE_DOMAIN"
	echo "                api.$NETMAKER_BASE_DOMAIN"
	echo "             broker.$NETMAKER_BASE_DOMAIN"
	echo "Make sure these domains are points to $SERVER_HOST to confirm the challenges"
	echo "-----------------------------------------------------"

	wait_seconds 5

	if [[ "$DOMAIN_TYPE" == "custom" ]]; then
		echo "Assuming DNS is configured correctly with records pointing to $SERVER_HOST"
	fi

	echo "-----------------------------------------------------"
	echo "Passing MASTER_KEY to Python .env file"
	echo "-----------------------------------------------------"
	echo "MASTER_KEY=$MASTER_KEY" >> /home/ubuntu/python/.env
	...
	echo "Generating MQ username..."
	
	MQ_USERNAME="netmaker"

	echo "Generating MQ password..."
	if [ -z "$MQ_PASSWORD" ]; then
		MQ_PASSWORD=$(tr -dc A-Za-z0-9 </dev/urandom | head -c 30)
	fi

	echo "Showing MQ credentials"
	echo "MQ Username: $MQ_USERNAME"
	echo "MQ Password: $MQ_PASSWORD"
	...
	echo "-----------------------------------------------------------------"
	echo "Confirming Settings for Installation"
	echo "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"
...
setup_mesh() {
	...
	echo "-----------------------------------------------------"
	echo "Passing TOKEN to Python .env file"
	echo "-----------------------------------------------------"
	echo "TOKEN=$TOKEN" >> /home/ubuntu/python/.env
	wait_seconds 3
